...
void CleanupFiles::SetDiskSpaceEmergency(bool enable, double checkThresholdGB, double deleteThresholdGB)
{
    m_enableDiskEmergency = enable;
    m_emergencyThresholdGB = checkThresholdGB;
    m_deleteTargetThresholdGB = deleteThresholdGB;
    std::string summary = std::string(u8"磁盘空间紧急清理 ") + (enable ? u8"已启用" : u8"已禁用") +
        u8" (检测阈值: " + std::to_string(checkThresholdGB) +
        u8" GB，目标阈值：" + std::to_string(deleteThresholdGB) + u8" GB)";
    Log(summary);
    m_messageCallback("EMERGENCY_CLEANUP", summary);
}
...
void CleanupFiles::CleanupTimeoutFilesFunction()
{
    ...
    while (!m_isExit)
    {
        // 1. 检查磁盘空间（紧急清理）- 每2分钟检查一次
        emergencyCheckCounter++;
        if (emergencyCheckCounter >= 2) // 每2分钟检查一次
        {
...
void CleanupFiles::ProcessEmergencyCleanup_old()
{
    ...
    while (!m_isExit && !spaceEnough)
    {
        ...
        if (deleted)
        {
            ...
            // 检查磁盘空间
            spaceEnough = CheckDiskSpace(true);

            if (!spaceEnough)
            {
                //Log(u8"已删除 " + std::to_string(foldersDeleted) +        u8" 个子文件夹，继续清理...");
            }
        }
        else
        {
    ...
    if (foldersDeleted > 0)
    {
        ...
        m_messageCallback("EMERGENCY_CLEANUP_OVER", u8"紧急清理完成: 共删除 " + std::to_string(foldersDeleted) +
            u8" 个子文件夹，释放了 " + std::to_string(freedGB) + u8" GB 空间");
        Log(u8"磁盘空间变化: 从 " + std::to_string(initialFreeGB) + u8" GB 增加到 " +
            std::to_string(finalFreeGB) + u8" GB (增加了 " +
            std::to_string(increasedGB) + u8" GB)");
        m_messageCallback("EMERGENCY_CLEANUP_OVER", u8"磁盘空间变化: 从 " + std::to_string(initialFreeGB) + u8" GB 增加到 " +
            std::to_string(finalFreeGB) + u8" GB (增加了 " +
            std::to_string(increasedGB) + u8" GB)");
    }
    else
...
void CleanupFiles::ProcessEmergencyCleanup()
{
    ...
    {
        Log(u8"未启用紧急清理功能，紧急清理终止");
        m_messageCallback("EMERGENCY_CLEANUP_OVER", u8"紧急清理结束，未启用紧急清理功能，紧急清理终止");
        return;
    }
    Log(u8"开始紧急清理流程");

    // 获取主目录路径
    std::filesystem::path mainDir(m_mainPath);
    if (!std::filesystem::exists(mainDir)) {
        Log(u8"主目录不存在，紧急清理终止");
        m_messageCallback("EMERGENCY_CLEANUP_OVER", u8"紧急清理结束，主目录不存在，紧急清理终止");
        return;
    }

    // 获取初始剩余空间
    QStorageInfo storage(QString::fromStdString(m_mainPath));
    double initialFreeGB = storage.bytesAvailable() / (1024.0 * 1024.0 * 1024.0);

    // 获取所有子文件夹，按创建时间升序（最旧在前）
    auto subfolders = GetAllSubfoldersSortedByAge();
    if (subfolders.empty()) {
        Log(u8"紧急清理结束，没有子文件夹可清理");
        m_messageCallback("EMERGENCY_CLEANUP_OVER", u8"紧急清理结束，没有子文件夹可清理");
        return;
    }

    // 定义空间检查函数
    auto spaceOk = [this]() -> bool {
        QStorageInfo storage(QString::fromStdString(m_mainPath));
        qint64 availableBytes = storage.bytesAvailable();
        double availableGB = availableBytes / (1024.0 * 1024.0 * 1024.0);
        return availableGB >= m_deleteTargetThresholdGB;
        };

    // 定义文件后缀匹配函数（不考虑时间）
    auto isTargetExtension = [this](const std::filesystem::path& file) -> bool {
        if (m_fileExtensions.empty())
            return false;   // 无指定后缀，第一阶段不清理任何文件
    ...
    int processedCount = 0;
    int sleepTime = BASE_INTERVAL;
    double freedGB = 0.0;
    int foldersDeleted = 0;

    // 第一阶段：清理指定后缀的文件
    if (!m_fileExtensions.empty()) {
        Log(u8"第一阶段：清理指定后缀的文件");
        for (const auto& folder : subfolders) {
            if (spaceOk()) {
                Log(u8"空间已达到目标阈值，停止第一阶段清理");
                break;
            }
            ...
            for (const auto& entry : std::filesystem::recursive_directory_iterator(folder)) {
                ...
                if (isTargetExtension(entry.path())) {
                    try {
                        uintmax_t fileSize = std::filesystem::file_size(entry.path());
                        std::filesystem::remove(entry.path());
                        freedGB += fileSize / (1024.0 * 1024.0 * 1024.0);
                        //Log(u8"已删除文件: " + entry.path().string());
                        processedCount++;
                        AdaptiveThrottle(processedCount, sleepTime);
                    }
                    catch (const std::exception& e) {
                        Log(u8"删除文件失败: " + entry.path().string() + " - " + e.what());
                    }
                }
            }
        }
    ...
    // 第二阶段：删除整个子文件夹
    if (!spaceOk()) {
        Log(u8"第二阶段：删除整个子文件夹");
        subfolders = GetAllSubfoldersSortedByAge(); // 重新获取确保顺序（创建时间不变）
        for (const auto& folder : subfolders) {
            if (spaceOk()) {
                Log(u8"空间已达到目标阈值，停止第二阶段清理");
                break;
            }

            uintmax_t folderSize = GetFolderSize(folder);
            try {
                std::filesystem::remove_all(folder);
                freedGB += folderSize / (1024.0 * 1024.0 * 1024.0);
                foldersDeleted++;
                //Log(u8"已删除文件夹: " + folder.string());
                int ProessedCount2 = foldersDeleted + processedCount;
                AdaptiveThrottle(ProessedCount2, sleepTime);
            }
            catch (const std::exception& e) {
                Log(u8"删除文件夹失败: " + folder.string() + " - " + e.what());
            }
        }
    }

    // 最终统计
    QStorageInfo finalStorage(QString::fromStdString(m_mainPath));
    double finalFreeGB = finalStorage.bytesAvailable() / (1024.0 * 1024.0 * 1024.0);
    double increasedGB = finalFreeGB - initialFreeGB;

    // 输出汇总信息
    std::string summary = u8"紧急清理完成: 共删除 " + std::to_string(foldersDeleted) + u8" 个子文件夹，" +
        u8"共删除 " + std::to_string(processedCount) + u8" 个指定后缀文件，" +
        u8"合计释放了 " + std::to_string(freedGB) + u8" GB 空间";
    Log(summary);
    if (m_messageCallback)
    {
        m_messageCallback("EMERGENCY_CLEANUP_OVER", summary);
    }

    std::string spaceChange = u8"磁盘空间变化: 从 " + std::to_string(initialFreeGB) + u8" GB 增加到 " +
        std::to_string(finalFreeGB) + u8" GB (增加了 " +
        std::to_string(increasedGB) + u8" GB)";
    Log(spaceChange);
    if (m_messageCallback)
    {
        m_messageCallback("EMERGENCY_CLEANUP_OVER", spaceChange);
    }

    // 最终空间检查
    if (spaceOk())
    {
        summary = u8"紧急清理完成，剩余空间达到目标阈值";
        Log(summary);
        
    }
    else
    {
        summary = u8"紧急清理完成，但空间仍未达到目标阈值（可能无更多可删内容）";
        Log(summary);
    }
    m_messageCallback("EMERGENCY_CLEANUP_OVER", summary);
}

bool CleanupFiles::CheckDiskSpace(bool bValue) const
{
    if (!m_enableDiskEmergency || m_mainPath.empty())
    {
    ...
    try
    {
        ...
        if (!storage.isValid()) {
            Log(u8"警告: 无法获取磁盘空间信息。" + m_mainPath);
        ...
        if (bValue)
        {
			isEnough = freeGB >= m_deleteTargetThresholdGB; // 删除到目标阈值
        }
        else
        {
			isEnough = freeGB >= m_emergencyThresholdGB;    // 检查阈值
        }
...
std::string CleanupFiles::GetDiskSpaceInfo() const
{
    ...
    try {
        ...
        ss << u8"磁盘空间信息:\n";
        ss << u8"  检查磁盘分区: " << storage.rootPath().toStdString() << "\n";
		ss << u8"  清理文件路径: " << m_mainPath << "\n";
