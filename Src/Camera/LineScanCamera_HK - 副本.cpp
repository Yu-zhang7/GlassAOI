...
//    if (pUser == nullptr)
//    {
//        LogPrintf("ImageGrabCallBack: pUser is nullptr!");
//        return;
//    }
//
//    if (pData == nullptr || pFrameInfo == nullptr)
//    {
//        LogPrintf("ImageGrabCallBack:pData or pFrameInfo is nullptr!");
//        return;
//    }
//
//    //1.先进行指针转换
...
//    if (camera == nullptr)
//    {
//        LogPrintf("static_cast failed! camera is empty!");
//        return;
//    }
//    //2. 拷贝帧信息
//    if (camera->m_frameInfo != nullptr) 
//    {
//        delete camera->m_frameInfo;
//        camera->m_frameInfo = nullptr;
//    }
...
//    //3. 拷贝图像数据
//
//    auto data = new unsigned char[pFrameInfo->nFrameLen];
//    std::memcpy(data, pData, pFrameInfo->nFrameLen);
//    //LogPrintf("Camera[%d] ImageGrabCallBack. PushFrame.", camera->GetCameraId());
//
//    //转换成cv::mat
//    cv::Mat mat = cv::Mat(pFrameInfo->nHeight, pFrameInfo->nWidth, CV_8UC1, data).clone();
//    //cv::Mat mat(pFrameInfo->nHeight, pFrameInfo->nWidth, CV_8UC1);
//    //std::memcpy(mat.data, pData, pFrameInfo->nFrameLen);
//    //LogPrintf("Camera[%d] ImageGrabCallBack. PushFrame end.", camera->GetCameraId());
//
//    /* 保存图像到队列 */
//    if (!camera->PushFrame(std::move(mat))) // 使用基类的队列方法添加图像
//    {
//        // 队列已满，记录丢帧
//        LogPrintf("Camera[%d] frame queue is full, dropping frame", camera->GetCameraId());
//    }
//    //LogPrintf("Camera[%d] ImageGrabCallBack. frameCount=%d", camera->GetCameraId(), camera->GetFrameCount());
//
//}
...
//
//void LineScanCamera_HK::GetMat(cv::Mat& img)
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    if (!m_imgList.empty())
//    {
//        img = m_imgList.back().clone();
//    }
//    else
//    {
//        img.release();
...
//}
//
//void LineScanCamera_HK::GetMatList(std::vector<cv::Mat>& imgList)
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    imgList = m_imgList;
//    m_imgList.clear();
...
//
//void LineScanCamera_HK::AddImageToQueue(cv::Mat& img)
//{
//    std::unique_lock<std::mutex> lock(m_saveMutex);
//    m_saveQueue.push(img);
//    m_saveCv.notify_one();
//}
//
//// 以下函数在基类中实现
//bool LineScanCamera_HK::GetReadyBatch(std::vector<cv::Mat>& batch)
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
...
//}
//
//int LineScanCamera_HK::GetReadyBatchCount()
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    return static_cast<int>(m_batchQueue.size());
//}
//
//void LineScanCamera_HK::GetMatListRange(std::vector<cv::Mat>& imgList, int num)
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    imgList.clear();
//    if (m_imgList.size() < static_cast<size_t>(num))
//        return;
//
//    for (int i = 0; i < num; i++)
//    {
//        imgList.push_back(m_imgList[i].clone());
//    }
//}
//
//void LineScanCamera_HK::ClearBuffers()
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    m_imgList.clear();
//    m_batchQueue.clear();
//}
//
//bool LineScanCamera_HK::GetLastBatch(std::vector<cv::Mat>& lastBatch)
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    if (m_imgList.empty())
//        return false;
//
//    lastBatch = std::move(m_imgList);
//    return true;
//}
//
//void LineScanCamera_HK::DataRelease()
//{
//    std::lock_guard<std::mutex> lock(m_batchMutex);
//    m_triggerNum = 0;
//    m_imgList.clear();
//}
//
//int LineScanCamera_HK::GetTriggerNum()
//{
//    return m_triggerNum.load();
//}
//
...
//
//int LineScanCamera_HK::getImageCount()
//{
//    return m_imgList.size();
//    //return 0;
//}
//
//bool LineScanCamera_HK::getImageFromList(cv::Mat& outImage)
//{
//    int imgIndex = m_imgList.size() - 1;
//    if (imgIndex < 0)
//    {
//        return false;
//    }
//    outImage = m_imgList[imgIndex];
//    if (outImage.empty())
//    {
//        return false;
//    }
